...
export default function StoryCard({ story, featured = false }: StoryCardProps) {
  ...
  return (
    <Link href={href} className="group block">
      <article
        className={`bg-white rounded-xl overflow-hidden border border-gray-200 hover:border-earth-gold/40 hover:shadow-lg transition-all duration-300 ${
          featured ? "md:flex" : ""
        }`}
      >
        {/* Cover image */}
          ...
          {story.cover_image_url ? (
            ...
            <div className="w-full h-full bg-gray-50 flex items-center justify-center">
              <div className="text-center px-6">
                <div className="w-16 h-16 mx-auto mb-3 rounded-full bg-earth-gold/10 flex items-center justify-center">
                  <svg className="w-8 h-8 text-earth-gold/50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className={`p-5 ${featured ? "md:w-1/2 md:p-8 md:flex md:flex-col md:justify-center" : ""}`}>
          ...
          <h3
            className={`font-serif font-bold text-gray-900 group-hover:text-earth-gold transition-colors ${
              featured ? "text-xl md:text-2xl" : "text-lg"
            }`}
          >
            {story.honoree_name}
          </h3>
          <h4 className="text-gray-500 text-sm mt-1 font-medium">{story.title}</h4>
          {story.summary && (
